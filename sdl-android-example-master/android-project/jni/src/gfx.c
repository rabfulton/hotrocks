...
void draw_powerup(powerup *p){
	...
	GLfloat offset = p->type * 0.25;
	GLfloat width = 0.25;
	GLfloat ptex[] = {offset, 1, offset, 0, offset + width, 1, offset + width, 0};
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glEnable(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, powertex);
	GLfloat box[] = { 	p->position.x - 15, p->position.y + 15,
						p->position.x - 15, p->position.y - 15,
						p->position.x + 15, p->position.y + 15,
						p->position.x + 15, p->position.y - 15,};
